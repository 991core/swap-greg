...
it("shows the seconds left and switches to the refresh animation at the deadline", async () => {
  const props = { expiresAt: Date.now() + 30_000, retryAt: null, loading: false, waiting: false, suspended: false };
  const view = render(<I18nProvider><QuoteCountdown {...props} /></I18nProvider>);
  expect(screen.getByRole("timer").getAttribute("aria-label")).toBe("Actualisation automatique dans 30s");
  await act(() => vi.advanceTimersByTimeAsync(1000));
  expect(screen.getByRole("timer").textContent).toContain("29s");
  await act(() => vi.advanceTimersByTimeAsync(29_000));
  expect(screen.getByRole("timer").className).toContain("is-refreshing");
  view.rerender(<I18nProvider><QuoteCountdown {...props} expiresAt={Date.now() + 30_000} /></I18nProvider>);
  expect(screen.getByRole("timer").textContent).toContain("30s");
});
